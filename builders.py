...
class Builders(object):
    ...
    def create_builder(self, repo_name):
        ...
        factory.addSteps([
            steps.ShellCommand(
                command=["rm", "-rf", venv_name], haltOnFailure=True,
        ...
        repo_index = [repo[order_repos_index]
                      for repo in self.repos if repo[0] is repo_name][0]
        repo_info = sorted(
            self.repos,
            key=lambda repo: repo[order_repos_index])[0:repo_index]
        for repo_name, repo_branch, _, namespace, in repo_info:
            _add_repo_to_factory(factory, repo_name,
                                 repo_branch, namespace, venv_name)

        factory.addSteps([
            steps.ShellCommand(
                command=['pep8', '.'],
                env=venv_path_factory, haltOnFailure=False,
                workdir=repo_name,
        ...
        if namespace is not '':
            ...
            factory.addSteps([
                ...
                steps.ShellCommand(
                    ...
                    doStepIf=(
                        lambda step: self.slaves.is_leap(
                            step.getProperty('slavename'))))
            ])

        self._publish_leap_wheels(
            factory, repo_name, venv_path_factory,
            doStepIf=(
                lambda step: self.slaves.is_leap(
                    step.getProperty('slavename'))))
        ...
        return BuilderConfig(
            name=builder_name, slavenames=self.slaves.names(), factory=factory)

    def _publish_coverage_reports_command(self, location, repo_name):
        coverage_dir = self.config.get(
            'ftp', 'coverage_reports_target_directory')
        target_directory = (
            coverage_dir +
            '/' + repo_name + '_' + '`git -C ' + repo_name + ' describe`')
    ...
    def _publish_leap_wheels_command(self, repo_name, location):
        ...
        link = self._ftp_soft_link(
            location, directory, 'leap.' + repo_name + '-latest.whl')
        command = ftp + ' && ' + link
        return command
    ...
    def _publish_leap_wheels_soledad(self, subpackage, location):
        ...
        ftp = self._ftp_publish_command(location, directory)
        link = self._ftp_soft_link(
            location, directory, 'leap.soledad.' + subpackage + '-latest.whl')
        command = ftp + ' && ' + link
        return command
    ...
    def _ftp_publish_command(self, from_location, to_location):
        ...
        ssh_command = [
            ...
            '"chmod -R g+r ' + (
                to_location + ' && chown -R ' +
                user + ':www-data ' + to_location + '"')]

        # Flatten to a string so that a shell executes de command, and
        # expands ~
        return (' '.join(ssh_mkdir_command) + ' ; ' +
                ' '.join(scp_command) + ' && ' +
                ' '.join(ssh_command))
    ...
    def make_wheel_builder(self):
        ...
        sandbox_path_soledad = {'PATH':  "../../" +
                                venv_name + '/bin/' + ':${PATH}'}

        factory.addStep(steps.ShellCommand(
            command=["virtualenv", "--python=python2", venv_name],
            haltOnFailure=True,
            workdir=".", name="Create new virtualenv"))

        factory.addStep(steps.ShellCommand(
            command=['pip', 'install', '-U', 'wheel'],
            env=sandbox_path_top,
            haltOnFailure=True,
            workdir=".", name="Install wheels"))
        for repo_name, git_branch, _, _ in self.repos:
            ...
            factory.addStep(
                steps.Git(
                    ...
                    workdir=workdir, mode='full', method='clobber',
                    shallow=True,
                    haltOnFailure=True,
                    name="Pull " + repo_url))
            if 'soledad' in repo_name:
                for subpackage in ["common", "client", "server"]:
                    factory.addStep(steps.ShellCommand(
                        command=generate_wheels,
                        env=sandbox_path_soledad,
                        haltOnFailure=True,
                        workdir=workdir + '/' + subpackage,
                        name="wheels for " + repo_name + "." + subpackage))
            else:
                factory.addStep(
                    steps.ShellCommand(
                        command=generate_wheels,
                        ...
                        haltOnFailure=True,
                        workdir=workdir,
                        name="wheels for " + repo_name))
        factory.addStep(
            steps.ShellCommand(
                command=publish_wheels,
                env=sandbox_path,
                doStepIf=(
                    lambda step: self.slaves.is_leap(
                        step.getProperty('slavename'))),
    ...
    def _add_pyside_setup_repo(self, factory):
        ...
        factory.addSteps([
            steps.ShellCommand(
                command=['rm', '-rf', repo_name], workdir='.',
                env=sandbox_path, name="Remove previous pyside"),
            steps.Git(
                repourl=repo_url, branch=git_branch,
                workdir=repo_name, mode='full',
                method='clobber', shallow=True,
                haltOnFailure=True,
                name="Pull " + repo_url),
            steps.ShellCommand(
            ...
            steps.ShellCommand(
                command=publish_pyside_wheel,
                workdir=repo_name + '/dist/', name="Publish pyside")
        ])
    ...
    def make_bundler_builder(self):
        ...
        factory.addSteps([
            steps.Git(
                repourl=repo_url, branch=branch,
                workdir=repo_dir, mode='full',
                method='clobber', shallow=True,
                haltOnFailure=True,
                name="Pull " + repo_url),
            steps.ShellCommand(
                command="rm -rf " + bundler_output_dir,
                workdir=workdir, name="Remove previous bundler dir"),
            steps.ShellCommand(
                command="mkdir " + bundler_output_dir,
                workdir=workdir, name="Create bundler dir"),
            steps.ShellCommand(
                command="cp bundle_pyinstaller.sh ../" + bundler_output_dir,
                workdir=repo_dir, haltOnFailure=True,
            ...
            steps.ShellCommand(
                ...
                name="Download sumo"),
            steps.ShellCommand(
                command="./bundle_pyinstaller.sh " + sumo_tarball,
                workdir=workdir + '/' + bundler_output_dir,
                name="Create bundle"),
            steps.ShellCommand(command=publish_bundle, workdir=workdir +
                               '/' + bundler_output_dir, name="Publish bundle")
        ])
    ...
    def _publish_bundle_command(self, location):
        ...
        ftp = self._ftp_publish_command(location, directory)
        link = self._ftp_soft_link(
            location, directory, 'bitmask-latest.tar.gz')
        command = ftp + ' && ' + link
        return command
...
def _add_repo_to_factory(factory, repo_name, git_branch, namespace, venv_name):
    ...
    if 'soledad.git' in repo_url:
        ...
        factory.addSteps([
            steps.ShellCommand(
                command=install_requirements, env=sandbox_path,
                haltOnFailure=False, workdir=workdir,
                name="reqs: " + repo_name),
            steps.ShellCommand(
                command=install_requirements_tests,
                ...
                workdir=workdir,
                name="test reqs: " + repo_name),
            steps.ShellCommand(
                command=install, env=sandbox_path,
